...
summary = {}
fields = ["target", "condition", "n_total", "n_success",
...
for (target, cond), rs in sorted(cells.items()):
    ...
    cost = stat([r.get("total_cost_usd") for r in succ])
    toks = stat([tot_tokens(r) for r in succ])
    turns = stat([r.get("num_turns") for r in succ])
    row = {
        "target": target, "condition": cond,
        "n_total": n, "n_success": len(succ),
...
for target in sorted({t for (t, _) in summary}):
    ...
    line = f"  {target}: 비용 {ratio('cost_median')}, 토큰 {ratio('tokens_median')}, 턴 {ratio('turns_median')}"
    if c2["capped_rate"] > 0:
        line += f"  (C2 capped {c2['capped_rate']*100:.0f}% → 최소 3배 잠정결론 후보)"
    print(line)
print()
