...
두 사람의 DISC 조합(`3⁴ × 3⁴ = 6,561`가지)은 캐싱 효율이 낮고 정확도가 신뢰도에 직결되므로
**매번 실시간 LLM 호출**을 원칙으로 하되, 사용자 대기를 없애기 위해 비동기로 처리합니다.
