...
## 🔬 설계 우선 원칙의 실증 · Design-First, Measured

> **설계 문서를 독립 변인으로 한 실험 설계**

"설계가 코드에 선행한다"는 개발 원칙이 실제로 효율을 낳는지, 주장에 그치지 않고 **통제 실험으로
측정**했습니다.

**변인 통제가 핵심입니다.** 두 조건 모두 동일하게 **TDD로 개발**합니다 — 기준 테스트가 구현의
근거로 항상 주어집니다. 유일한 차이는 그 TDD 과정을 **설계 문서가 보조하는가**입니다. 나머지
(스텁 코드·프롬프트·모델·기준 커밋)를 모두 고정하고, 대상 서비스의 **히스토리 제거 사본**으로 문서
접근을 원천 차단해, **독립 변인을 오직 '설계 문서의 존재' 하나로 한정**했습니다.

- **C1** = TDD + 설계 문서 보조 · **C2** = 설계 문서 없는 TDD(기준선)
- AI 에이전트(Claude Sonnet)에게 스텁 구현 과제를 **조건당 10회(총 40회) 반복**시키고, 완료까지의
  비용·시간·토큰을 비교. 각 지표에 **Shapiro-Wilk 정규성 검정** 후 **Welch t-검정** 또는 **Mann–Whitney U**를 선택 적용

**측정 결과 — 효과는 코드의 성격에 의존했습니다.**

- **순수 도메인 로직**(채점·버킷 변환): 문서 효과가 **관측되지 않았습니다.** 테스트와 프롬프트만으로
  동작이 온전히 결정되는 코드에는 설계 문서가 더할 것이 없습니다.
- **동시성·캐시 무효화·실패 복구가 얽힌 코드**(케미 캐시): 문서 有가 **모든 지표에서 저렴·빠름**
  (중앙/평균 기준 약 7–13% 적게)이라는 방향이 일관되게 나타났습니다. 다만 조건당 n=10에서는
  **통계적 유의에는 이르지 못해**(전 지표 p≥.05, 로컬 도구시간만 marginal) 단정은 보류합니다.

즉, **설계 문서는 설계 의도가 코드·테스트 밖 문서에 흩어져 있을 때 TDD를 보조해 효율을 높입니다.**
"무조건 문서화가 이득"이 아니라, *어디에 설계 문서가 필요한지*에 대한 실증적 근거입니다.

> **한계를 명시합니다.** 측정 모델은 Sonnet에 한정되며, 대상 2종·조건당 n=10의 소규모 실험입니다.
> 지표의 방향은 일관되나 표본이 작아 비용 우위를 단정하지 않습니다. 실험 설계·방법론·전체 결과와
> 통계는 [`benchmark/report/benchmark-report.md`](benchmark/report/benchmark-report.md)에 있습니다.
