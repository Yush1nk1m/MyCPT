...
- AI 에이전트(Claude Sonnet)에게 스텁 구현 과제를 **48회 반복**시키고, 완료까지의 비용·시간·토큰을
  Mann–Whitney U 검정으로 비교

**측정 결과 — 효과는 코드의 성격에 의존했습니다.**

- **순수 도메인 로직**(채점·버킷 변환): 문서 효과가 **관측되지 않았습니다.** 테스트와 프롬프트만으로
  동작이 온전히 결정되는 코드에는 설계 문서가 더할 것이 없습니다.
- **동시성·캐시 무효화·실패 복구가 얽힌 코드**(케미 캐시): 문서 有가 **시간·토큰 기준 약 10–18%
  적게** 소모했습니다(비용도 같은 방향이나 통계적으로는 경계선).

즉, **설계 문서는 설계 의도가 코드·테스트 밖 문서에 흩어져 있을 때 TDD를 보조해 효율을 높입니다.**
"무조건 문서화가 이득"이 아니라, *어디에 설계 문서가 필요한지*에 대한 실증적 근거입니다.

> **한계를 명시합니다.** 측정 모델은 Sonnet에 한정되며, 대상 2종·조건당 n=10–13의 소규모 실험입니다.
