...
Spring Security 세션 쿠키 기반. 카카오 로그인 완료 후 `JSESSIONID` 발급.
...
- 인증: 비회원
- 설명: 카카오 로그인 완료 후 콜백을 처리한다. 신규 회원이면 가입 처리 후 세션을 발급하고, 기존 회원이면 세션을 발급한다. 처리 완료 후 프론트 `/`로 리다이렉트한다.
...
- 인증: 회원 전용
- 설명: 현재 세션을 무효화하고 세션 쿠키를 만료시킨다. 페이지 이동은 클라이언트가 처리한다.
- 요청 바디: 없음
...
- 인증: 회원 전용
- 설명: 현재 세션의 회원 정보를 반환한다. 페이지 로드 시 로그인 상태 확인 및 헤더 렌더링에 활용.
