...
| v0.8 | DISC Cache에 선제적으로 스크립트 실행하여 버킷을 추가하는 시드 로직 추가                                                                                    | '26.06.03  |
...
- `disc_cache`는 초기화 스크립트에 의해 3^4=81개 행이 `report=NULL`로 사전 삽입됨. LLM 보고서 미생성 상태를 `report=NULL`로 표현하며, 최초 조회 시 LLM을 호출해 UPDATE함. 이로써 `disc_results → disc_cache` FK 제약이 항상 만족됨.
