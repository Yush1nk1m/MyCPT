...
| v0.2 | §4에 "캐시 즉시 반영" 시나리오 4종 신설(`E2E-Cache-*`) — 쓰기 후 다른 화면 진입 시 React Query 캐시 갱신 회귀 탐지. 새로고침 금지·1분 이내 확인 조건 명시 | '26.07.29. |
...
> ⚠ **모든 케이스에서 브라우저 새로고침을 하지 않는다.** 새로고침은 QueryClient를 재생성해 캐시를 비우므로, 새로고침하면 회귀를 탐지할 수 없다. 화면 이동만으로 확인한다.
> 전역 `staleTime`이 1분이므로 **쓰기 직후 1분 이내**에 확인해야 한다. 규약은 `frontend/component-map.md` §쿼리 키·캐시 무효화 규약.

| Test ID | 사전 조건 | 시나리오 | 연결된 API | 기대 결과 |
| --- | --- | --- | --- | --- |
| E2E-Cache-프로필→통계 | 생년·성별 미입력 회원, 검사 이력 1회 이상 | `/me/profile`에서 생년·성별 저장 → 즉시 `/me/insights` 진입 | `PATCH /users/me` → `GET /statistics/comparison` | "생년·성별을 입력하면…" 배너가 사라지고 또래 평균 막대·범례 표시 |
| E2E-Cache-검사→통계이력 | 검사 이력 1회 이상 회원 | 검사 완료·저장 → 즉시 `/me/insights`·`/results` 진입 | `POST /results` → `GET /statistics/*`, `GET /results` | 방금 응시분이 추이·이력에 반영 |
| E2E-Cache-발행→코인 | 코인 보유 회원, 발행 가능 동료 | 케미 보고서 발행 → 즉시 헤더 코인 알약과 `/me` "코인 N개" 확인 | `POST /chemistry` → `GET /coins` | 두 곳 모두 차감된 잔량으로 즉시 변경 |
| E2E-Cache-충전반영 | `coins=0`이고 `next_coin_at`이 경과한 계정 (시드로 조작) | 코인 화면을 거치지 않고 아무 페이지 진입 | `GET /coins` | 헤더가 온디맨드 충전이 반영된 잔량 표시 (`/auth/me`는 충전하지 않으므로 회귀 시 0 유지) |
