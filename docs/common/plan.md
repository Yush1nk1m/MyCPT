...
- [x] 카카오톡 공유 시 intent:// 스킴 오류로 실패 (로컬 개발 환경 특성인지 실제 앱키/도메인 등록 이슈인지 원인 파악 필요)
- [x] 서비스 소개 시트 본문 텍스트 줄바꿈/가독성 개선
- [x] 케미 발행 NO_RESULT 에러 메시지가 실제 화면에 표시 안 됨 (onError 분기는 추가했으나 렌더링 확인 필요)
- [x] KakaoSdkLoader.tsx: Kakao.init() 인자 타입 오류 (NEXT_PUBLIC_KAKAO_JS_KEY: string | undefined)
...
| 07.06 (월) | 이월 버그 4건 해소 — about-sheet 줄바꿈 제어, 케미 NO_RESULT 에러 코드 파싱 누락 수정, KakaoSdkLoader TS 오류 해소, 카카오톡 공유 로직 useKakaoShare 훅으로 통합(동료 목록 스텁 버튼도 실제 SDK 연동으로 교체). 카카오 공유 링크 미표시는 localhost 환경 한계로 원인 규명, 배포 후 재검증으로 이월.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | useKakaoShare, ShareStep3Link, colleagues/page.tsx, AboutSheet, postChemistryReport                                                                                                                                           |
